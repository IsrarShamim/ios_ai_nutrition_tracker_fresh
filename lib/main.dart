...
import 'package:ai_nutrition_tracker_for_ios/data/subscriptions/SubscriptionChecker.dart';
...
Future<void> main() async {
  ...
  isar = await Isar.open(
    [
      UserInformationEntitySchema,
      NutritionSchema,
      NutritionGoalSchema,
      NutritionUploadStatusSchema,
      NutritionGoalHolderSchema,
      ChatMessageSchema,
      MealSchema,
      IndividualWaterLogSchema
    ], // Pass the schema of your entity
    directory: dir.path,
  );
  ...
  Widget build(BuildContext context) {
    return CupertinoApp.router(
      ...
      builder: (context, child) {
        // Wrap the child with Localizations
        return Localizations(
          ...
          delegates: [
            ...
            GlobalCupertinoLocalizations
                .delegate, // Needed for Cupertino widgets
          ],
...
Future<bool> isUserAuthenticated() async {
  try {
    final user =
        FirebaseAuth.instance.currentUser; // Check current Firebase user
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        ...
        child: const Icon(Icons.add,
            size: 36, color: Colors.white), // This ensures a circular shape.
      ),
...
final GoRouter router = GoRouter(
  ...
  redirect: (context, state) async {
    ...
    final isSubscribed = await SubscriptionChecker.isSubscribed();

    if (state.uri.toString() == '/splash') {
      if (!isAuthenticated) {
        return '/welcome';
      } else {
        return isSubscribed ? '/home' : '/subscription_plan';
      }
    }
    return null;
  },
  routes: <RouteBase>[
    ...
    GoRoute(
      ...
      pageBuilder: (context, state) => CustomTransitionPage(
        key: state.pageKey,
        child: OnboardingSubscribeScreen(
    ...
    GoRoute(
      ...
      pageBuilder: (context, state) => CustomTransitionPage(
        key: state.pageKey,
        child:
            RecipeGenerationScreen2(), // This screen should not have its own nav bar.
        transitionDuration: Duration.zero,
        reverseTransitionDuration: Duration.zero,
        transitionsBuilder: (context, animation, secondaryAnimation, child) {
          return child;
    ...
    GoRoute(
      ...
      pageBuilder: (context, state) => CustomTransitionPage(
        ...
        transitionDuration:
            const Duration(milliseconds: 400), // Smooth duration
